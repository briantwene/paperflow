...
impl OAuthSession {
    ...
    pub async fn wait_for_callback(self) -> AuthResult<String> {
        // Open auth window
        Self::open_auth_window(&self.app_handle, &self.auth_url).await?;
    ...
    async fn open_auth_window(app_handle: &tauri::AppHandle, auth_url: &str) -> AuthResult<()> {
        let webview_url = tauri::WebviewUrl::External(
            auth_url.parse()
                .map_err(|e| AuthError::Config(format!("Invalid auth URL: {}", e)))?
        );
        
        let window = tauri::WebviewWindowBuilder::new(app_handle, "reddit_auth", webview_url)
            .fullscreen(false)
            .resizable(true)
