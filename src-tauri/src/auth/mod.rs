...
pub async fn start_reddit_auth_v2(app_handle: tauri::AppHandle) -> Result<String, String> {
    use crate::auth::{oauth_session::OAuthSession, reddit_auth::RedditAuth};
    ...
    // Get auth URL
    let auth_url = auth
        .initiate_login(&state)
        .await
        .map_err(|e| format!("Failed to initiate login: {}", e))?;

    // Start OAuth session
    let oauth_session = OAuthSession::new(app_handle, auth_url)
        .await
        .map_err(|e| format!("Failed to create OAuth session: {}", e))?;

    // Wait for callback
    let auth_code = oauth_session
        .wait_for_callback(&state)
        .await
        .map_err(|e| format!("OAuth callback failed: {}", e))?;

    // Complete authentication
    auth.complete_auth(auth_code)
        .await
